...
def rmSeasonality(df_train):
    ...
    #plt.stem(freq, fftt)
    #plt.show()
    sd_24 = seasonal_decompose(df_train['detrendMain'].values, period=24)
    sd_48 = seasonal_decompose(df_train['detrendMain'].values.reshape(-1,1) - np.array(sd_24.seasonal).reshape(-1, 1), period=48)
    sd_week = seasonal_decompose(df_train['detrendMain'].values.reshape(-1,1) - np.array(sd_48.seasonal).reshape(-1, 1), period=24*7)
    sd_year = seasonal_decompose(df_train['detrendMain'].values.reshape(-1,1) - np.array(sd_week.seasonal).reshape(-1, 1), period=24*30*7)
    # drawing figure with subplots, predefined size and resolution
    f, axes = plt.subplots(6, 1, figsize=(80, 8))
    # setting figure title and adjusting title position and size
    plt.suptitle('Summary of seasonal decomposition', y=0.92)
    axes[0].plot(sd_year.trend[2000:10000])
    axes[0].set_title('Trend component')
    axes[1].plot(sd_24.seasonal[2000:10000])
    axes[1].set_title('Daily season')
    axes[2].plot(sd_48.seasonal[2000:10000])
    axes[2].set_title('2 Day season')
    axes[3].plot(sd_week.seasonal[2000:10000])
    axes[3].set_title('Weekly season')
    axes[4].plot(sd_year.seasonal[2000:10000])
    axes[4].set_title('Months component')
    axes[5].plot(sd_year.resid[2000:10000])
